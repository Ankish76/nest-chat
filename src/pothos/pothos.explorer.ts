...
import {
  ...
  InternalPothosRefDecoratorToken,
} from './constants';
...
export class PothosDecoratorExplorer {
  ...
  getMetadataOfDecoratedMethods(target: any) {
    ...
    return this.metadataScanner
      .scanFromPrototype(target, instancePrototype, (method) =>
        this.exploreMethodMetadata(target, method),
      )
      .filter(
  ...
  protected exploreMethodMetadata(
    instance: any,
    methodKey: string,
  ): PothosDecoratorMetadata | null {
    const targetCallback = instance[methodKey];
    const metadata: PothosDecoratorMetadata | null = Reflect.getMetadata(
      InternalPothosDecoratorToken,
      targetCallback,
    );
    ...
    return {
      ...metadata,
      isPothosRefDecorator: !!Reflect.getMetadata(
        InternalPothosRefDecoratorToken,
        targetCallback,
      ),
    };
